...
def runExperiment(hydraConfig: dict):
    ...
    if chosenBenchmark == "Branin":
        ...
        parameterlist = [
            ...
            RangeParameter(
                name="x5", parameter_type=ParameterType.FLOAT, lower=0, upper=1
            ),
            RangeParameter(
                name="x6", parameter_type=ParameterType.FLOAT, lower=0, upper=1
            ),
        ]
    ...
    if executeTraining:
        ...
        for _ in range(numberOfSobolRounds):
            trial = exp.new_trial(generator_run=sobol.gen(1))
            trial.run()
            trial.mark_completed()

        # Botorch-Traningrounds
        for _ in range(numberOfBotorchRounds):
